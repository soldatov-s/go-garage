...
func (conn *Enity) workWithQueue() bool {
	...
	if err != nil {
		conn.log.Error().
			Err(err).
			Msg("Failed to start SQL transaction. Items prepared to be pushed to PostgreSQL will be pushed back to queue")
		conn.queueMutex.Lock()
	...
	if err1 != nil {
		...
		conn.log.Error().Err(err1).Msg("Failed to commit transaction to PostgreSQL, rolling back...")
