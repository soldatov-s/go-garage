...
func Registrate(ctx context.Context) context.Context {
	return providers.RegistrateByName(ctx, ProvidersName, NewMessagingServers(ctx))
}

func Get(ctx context.Context) *MessagingServers {
	v := providers.GetByName(ctx, ProvidersName)
	if v != nil {
		return providers.GetByName(ctx, ProvidersName).(*MessagingServers)
	}
	return nil
}
